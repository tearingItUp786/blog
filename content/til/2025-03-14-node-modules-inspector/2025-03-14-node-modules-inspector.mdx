---
title: Node Modules Inspector
date: 2025-03-14
tag: Node
---

TIL about the `node-modules-inspector` -- a package that allows you to visualize your node_modules folder,
inspect your dependencies, and so much more 🤯! I'm a big fan of the visulaizations, since I'm more of a
visual kinda person. You can use it in your own project with this simple command: `npx node-modules-inspector`.

[Check it out here](https://github.com/antfu/node-modules-inspector)
