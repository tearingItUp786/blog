...
Imagine that the computer were the driver of a car and you were the passenger. Your driver happens to be well meaning
but has a terrible sense of direction so you are tasked with providing directions on how to 
get to your house (every left and right turn and every highway/exit that would be needs to be taken). 
...
unlike the imperative example where the driver had no sense of direction it would be up to the driver's responsbility
to figure out the route. All you really care about the end result, that is, you ending up at home (hopefully there’s a cute puppy there to 
greet you though because who doesn’t love being greeted by a puppy? 😍).

Functional programming is a declarative style of programming. This means that the code, at least in theory, that
...
programming, there’s one thing that both camps can agree on: all things being equal, code 
...
const names = users.map(getUpperCaseUserName);
```
...
reserve some space in their brain for this variable because it will show when this things shows up later on in the program. 
...
the `getUpperCaseUserName` function (gee, I wonder what that function does? -.-). Assuming you know that the map function 
on the Array prototype applies a transformation function (the argument to `.map`) and returns a new array with the newly
transformed values, you would agree that the second, declarative example is more readable and maintainable. 