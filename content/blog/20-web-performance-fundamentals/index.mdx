...
1. 🖼️ _Largest Contentful Paint (LCP)_: how fast the largest visible element loads in. This tends to be images, videos, css background images, text elements. Basically,
...
3. ⌨️ _Interaction to Next Paint (INP)_: how responsive your page is to user interactions (keyboard, clicks, but not scrolling). Basically,
...
Let's jump into the meat 🍖 and potatoes 🥔 of this post! Let's talk about how to make some gains 💪!
For anyone reading this later (including myself), try to keep the following in mind:

1. 🧘 Focus on the _easiest_ fixes for your _worst_ metrics from real user data, where possible.
2. 🤔 Don't try to do everything. Sometimes we're fast enough.
3. ⚡ The most basic rule of performance.... **do fewer things**.
...
side of the performance question 🛣!
...
Another method we can use to improve TTFB is by utilize efficient protocols when delivery our content! Refer to the below table.

| Feature / Constraint    | HTTP/1.1 🚗                                 | HTTP/2 🚅                                                                   | HTTP/3 ✈️                                                            |
| ----------------------- | ------------------------------------------- | --------------------------------------------------------------------------- | -------------------------------------------------------------------- |
| Transport               | TCP                                         | TCP                                                                         | QUIC (UDP)                                                           |
| Request Handling        | Sequential; multiple TCP connections needed | Multiplexed streams over a single TCP connection                            | Multiplexed streams over QUIC with no TCP “head-of-line” blocking    |
| Parallel Requests Limit | ~6–10 TCP connections per domain            | Usually 1 connection per domain; streams limited by server (10–100 typical) | 1 QUIC connection per domain; streams limited by server (often >100) |
...
The TLDR of that table, if you didn't go through is the following 😅:

- HTTP/1.1 🚗 = A single-lane road — one car at a time.
- HTTP/2 🚙 = A multi-lane road — many cars, but if one crashes, traffic slows.
- HTTP/3 🚀 = A multi-lane road with smart lanes — if one crashes, others keep moving.

So, if you're having with TTFB, look at _how_ you're delivering content to your users and talk to your ops engineers -- if you have
ops engineers that is 👀!

### First Contentful Paint

First Contentful Paint (FCP) is how fast our site loads **something** -- anything really. CSS and Fonts are
**render blocking** by default so when you're including those types of assets, the prevent the page from rendering
until they complete. This is why [minification 📰](<https://en.wikipedia.org/wiki/Minification_(programming)>) and [bundling 🎥](https://youtu.be/5IG4UmULyoA) are important for these kinds of assets!
...
What we want to do here is utilize strategics like preloading resources wherever possible. The following can be preloaded via the
...
loading in resources only as they are needed, think on user scroll or perhaps after a button press, loading in some
...
- 🎲 **Async** -- Downloads in parallel with HTML parsing, executes immediately when downloaded, pausing parsing.
  Execution order is unpredictable.

- ⏳ **Defer** - Downloads in parallel with HTML parsing, executes after parsing is complete, right before DOMContentLoaded.
...
you know that `images` and `iframes` accept a [loading attribute](https://developer.mozilla.org/en-US/docs/Web/Performance/Guides/Lazy_loading#images_and_iframes)? Well, they do ⚡, so we can set this
...
accomplished by, assuming the resource is an image or something to that affect, by using a `<link>` tag to preload our image asset...
...
at an acceptable quality level. You might be saying, but bro 🤨, didn't we already talk about compressing via Brotli or gzip,
...
formats like JPEG, PNG, or WebP. Running them through gzip won't magically shrink them more — in fact, it usually does nothing 😅.
...
| Format  | Compression    | Typical Size 📦       | Quality 🖼️                                      | Browser Support 🌐            | Best Use Case                             |
| ------- | -------------- | --------------------- | ----------------------------------------------- | ----------------------------- | ----------------------------------------- |
| JPEG 📸 | Lossy          | Medium–Small          | Good, but can show artifacts                    | Universal ✅                  | Photos, complex images                    |
| PNG 🖌️  | Lossless       | Large                 | Excellent (no artifacts, supports transparency) | Universal ✅                  | Logos, icons, images needing transparency |
| WebP 🚀 | Lossy/Lossless | Smaller than JPEG/PNG | High (supports transparency + animation)        | Widely supported (not IE)     | General purpose replacement for JPEG/PNG  |
| AVIF 🌈 | Lossy/Lossless | Smallest              | Excellent (HDR support)                         | Growing (all modern browsers) | Cutting edge for photos, hi-res graphics  |
...
<picture class="aspect-h-8 aspect-w-12 my-0 w-full">
	<!-- ≤600px screens → 480w (small phones) -->
	<source
	...
	<!-- 601–1080px screens → 800w (large phones / small tablets) -->
	<source
...
caching is something that can be discussed at length by people far more qualified than I 😅. Here's a great [talk](https://www.epicweb.dev/talks/caching-for-cash)
by my friend Kent that touches upon the topic.

### Cumulative Layout Shift and Interaction To Next Paint

Last but not least, let's cover our other two metrics. We've improved TTFB, FCP, and LCP — but if you've lazy-loaded a bunch
...
measures how fast users can interact — clicks, taps, typing.
...
At the end of the day, web performance isn't about chasing perfect scores — it's about respecting your users' time.
Shave off the waste, fix the biggest pain points first, and remember: fast is subjective and you're not really the one
